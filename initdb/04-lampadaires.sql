...
FROM Mobilier m
JOIN Lieu l
    ON m.fk_id_lieu = l.id_lieu
LEFT JOIN Interventions i
    ON  i.objet ILIKE '%lampadaire%'
    AND i.objet ILIKE '%' || l.nom || '%'
WHERE LOWER(TRIM(m.type)) = 'lampadaire'
GROUP BY
    m.id_mobilier,
    l.nom,
    m.materiau,
...
WITH cout_moyen AS (
    ...
    SELECT
        ROUND(AVG(i.cout_materiel), 2) AS cout_unitaire
    FROM Interventions i
    WHERE LOWER(i.type_intervention) = 'remplacement'
...
SELECT
    ...
    longitude
FROM cumul
WHERE cout_cumule_budget <= 50000
ORDER BY score_priorite DESC;
